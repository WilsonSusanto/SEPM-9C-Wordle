...
function compareWords(guessedWord, attempt) {
    ...
    if (guessedWord != wordOfTheDay) {
        ...
        for (let i = 0; i < 5; i++) {
            document.getElementById(String(attempt) + "-" + i).style.backgroundColor = "gray"
        }

        for (let i = 0; i < incorrectLocations.length; i++) {
            document.getElementById(String(attempt) + "-" + incorrectLocations[i]).style.backgroundColor = "orange"
        }
        for (let i = 0; i < correctLocations.length; i++) {
            document.getElementById(String(attempt) + "-" + correctLocations[i]).style.backgroundColor = "green"
        }
        return false;
    }
    else {
        for (let i = 0; i < 5; i++) {
            document.getElementById(String(attempt) + "-" + i).style.backgroundColor = "green"
        }
        return true;
    }
